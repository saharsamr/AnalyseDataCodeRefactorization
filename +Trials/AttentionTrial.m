classdef AttentionTrial < Trials.Trial
    ...
    methods (Access = public)
        ...
        function set_states_of_trail (this)
            ...
            this.set_cue_index();
            this.set_change_index();
            this.set_should_keep_index();
        end
    ...
    methods (Access = protected)
        ...
        function set_cue_index (this)
           cue_index = Utils.Util.find_all(this.states.info, 'cueIndex:');
           if ~isempty(cue_index)
               cue_index = cue_index(end);
               this.cue_index = Utils.Util.substr2double( ...
                                        this.states.info{cue_index}, ':', 2 ...
                );
           else
               warning(['no cueIndex found in trial events! setting it to 1!'])
               this.cue_index = 1;
           end
        end

        function set_change_index (this)
           change_index = Utils.Util.find_all(this.states.info, 'changeIndex:');
            if ~isempty(change_index)
                change_index = change_index(end);
                this.change_index = Utils.Util.substr2double( ...
                                        this.states.info{change_index}, ':', 2 ...
                );
            else
                warning(['no changeIndex found in trial  events! setting it to 1!'])
                this.change_index = 1;
            end
        end

        function set_should_keep_index (this) %TODO: functionality of this part has changed a bit. check it carefully.
            ...
            elseif Utils.Util.do_exist(this.states.info, 'stimulus_waiter=>releaseWait')
                ...
                warning(['no shouldKeepIndex found in trial  events! setting it to 0!'])
                this.should_keep = 0;
            end
        end
        ...
        function set_goodness_parameter (this) % TODO: check functionality in the end.
            ...
            this.primary_goodness = reward_state && no_error && ...
                                    (this.good_amount <= this.reward_value ...
            );
        end
        ...
        function set_reaction_time_and_update_goodness (this, time_data)
            if this.primary_goodness && this.should_keep == 0
                ...
                if start_state_index < temp_point_release
                    try
                        this.set_reaction_times(temp_point_release, start_state_index);
        ...
        function set_reaction_times (this, temp_point_release, start_state_index)
            release_point = find(this.bar.signal.bar( ...
                this.bar.signal.time > ...
                this.states.time(temp_point_release) - this.startTime ...
            )==0, 1);
            ...
            this.reaction_time = ...
                    release_point_time - ...
                    (this.states.time(tempFromTime) - start_state_index) ...
            ;
        end
